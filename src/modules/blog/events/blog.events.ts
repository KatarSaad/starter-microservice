...
export class BlogEventHandler {
  ...
  private readonly circuitBreaker = new CircuitBreaker(async (fn, args) => fn(...args), {
    timeout: 1000,
  ...
  async handleCreatedEvent(@Payload() data: any, @Ctx() context: RmqContext) {
    const result = await this.processEvent('created', data, context, this.blogService.create.bind(this.blogService));
  ...
  async handleUpdatedEvent(@Payload() data: any, @Ctx() context: RmqContext) {
    ...
    await this.processEvent('updated', { id, updateData }, context, this.blogService.update.bind(this.blogService));
  }

  @MessagePattern(BlogEventNames.Deleted)
  async handleDeletedEvent(@Payload() data: any, @Ctx() context: RmqContext) {
    await this.processEvent('deleted', data, context, this.blogService.delete.bind(this.blogService));
  }

  @MessagePattern(BlogEventNames.GetAll)
  async handleGetAllEvent(@Payload() data: any, @Ctx() context: RmqContext) {
    const result = await this.processEvent('getAll', data, context, this.blogService.findAll.bind(this.blogService));
    return result; // Return result after processing
  }
  ...
  async handleGetEvent(@Payload() data: any, @Ctx() context: RmqContext) {
    const { id } = data;
    const result = await this.processEvent('get', id, context, this.blogService.findOne.bind(this.blogService));
    return result; // Return result after processing
  }

  private async processEvent(event: string, data: any, context: RmqContext, serviceMethod: (...args: any[]) => Promise<any>) {
    try {
  ...
  private handleError(context: RmqContext, error: any, event: string, data: any) {
    const channel = context.getChannelRef();
    const originalMessage = context.getMessage();
    channel.nack(originalMessage, false, false);
    this.logger.error(`Failed to process '${event}' event for data: ${JSON.stringify(data)}`, error.stack);
